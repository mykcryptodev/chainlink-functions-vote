...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function executeRequest(
    ...
    functionRequestsForProposals[assignedReqID] = args[0];
  ...
  function executeProposalWithIndex(
    ...
    require(keccak256(abi.encodePacked(to, value, data, operation)) == proposals[proposalId][0], "Tx hash mismatch");
  ...
  // function test(string memory proposalId) public {
  //   (address to, uint256 value, bytes memory txData, Operation operation) = decodeTx(proposals[proposalId]);
  //   console.log("to %s", to);
  //   require(exec(to, value, txData, operation), "Module transaction failed");
  // }
  ...
  function stringsAreEqual(string memory s1, string memory s2) public view returns (bool) {
    console.log("s1 %s", s1);
    console.log("s2 %s", s2);
  ...
  function txIndexToExecute(string memory proposalId) public view returns (uint256) {
    return 0;
  }
